...
const Products = () => {
  ...
  const [filteredData, setFilteredData] = useState([]);
  const navigate = useNavigate();
  const backendUrl = "http://localhost:30000"; // Replace with your actual backend URL

  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`${backendUrl}/api/food/getall`);
        setData(res.data);
  ...
  const addToCart = (item) => {
    ...
    const isDuplicate = existingCart.some((i) => i._id === item._id);
  ...
  const handleRatingChange = async (productId, newRating) => {
    ...
    if (!userId) {
      toast.warn("Please log in to rate products.");
      navigate("/login");
      return;
    }

    try {
      const res = await axios.post(`${backendUrl}/api/food/rateProduct`, {
        productId,
        userId,
        rating: newRating,
      });
    ...
    <div className="bg-gray-900 min-h-screen text-white p-4">
      ...
      <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {filteredData.map((item) => (
          <div
            key={item._id}
            className="bg-gray-800 p-4 rounded-xl shadow-md hover:shadow-lg transition duration-200"
          >
            <img
              src={item.image}
              alt={item.name}
            ...
            <div className="mt-3">
              <ReactStars
                ...
                activeColor="#ffd700"
                onChange={(newRating) =>
                  handleRatingChange(item._id, newRating)
                }
              />
              <span className="text-sm text-gray-300">
                {item.rating ? item.rating.toFixed(1) : "No ratings yet"} / 5
              </span>
            </div>
